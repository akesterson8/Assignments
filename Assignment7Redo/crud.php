	...
	/*THIS FUNCTION TAKES THE DATA FROM THE DATABASE AND RETURN AN UNORDERED LIST OF THE DATA*/
	 public function createList($records){
		...
		foreach ($records as $row){
            ...
            $list .= "<li> <a href = \"http://russet.wccnet.edu/~akesterson/Assignment7Redo/List/$filename\">file $count </a></li>";
            $count++;
			//$list .= "<li> {$row['id']} - file_name: {$row['file_name']} - file_path: {$row['file_path']} }</li>";
		}
		$list .= '</ol>';
		return $list;
	}

	/*THIS FUNCTION TAKES THE DATA AND RETURNS THE DATA IN INPUT ELEMENTS SO IT CAN BE EDITED. not needed */
