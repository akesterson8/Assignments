...
class Validation{
	...
	private function address($value){
		$match = preg_match('/^\d*[-\d]+\s+[-#a-z\s\'\.\d]+$/i', $value);  
		return $this->setError($match);
	}
	private function city($value){
		$match = preg_match('/^[-a-z-\'\s]{1,100}$/i', $value); 
		return $this->setError($match);
	}
	...
	private function email($value){
		$match = preg_match('/^([a-z0-9_\.-]+)@([\da-z\.-]+){3}\.([a-z\.]{3})$/i', $value); 
		return $this->setError($match);
	}
	private function dob($value){
		$match = preg_match('/^\d{2}\/\d{2}\/\d{4}$/', $value);
		if($match){
			$mm = substr($value,0,2); // echo($mm);
			$dd = substr($value,3,2); // echo($dd);
			$yyyy = substr($value,6,4); // echo($yyyy);
